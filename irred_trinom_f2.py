...
"""
We represent the polynomial f(x) == x^a + x^b + ... by evaluating it at x == 2.
For example, 1 + x^2 + x^3 is 0b1101.
Addition of two polynomials is the bitwise xor of the corresponding numbers,
and multiplication is the xor-product from Project Euler #810.
"""
...
def xordivmod(N, D):
    ...
    ndeg, ddeg = N.bit_length() - 1, D.bit_length() - 1
    if ndeg < ddeg: return (0, N)
    q = 0
    while ndeg >= ddeg:
        q ^= 1 << (ndeg - ddeg)
        N ^= D << (ndeg - ddeg)
...
def xorgcd(f, g):
    df, dg = f.bit_length() - 1, g.bit_length() - 1
    while g != 0:
        r = xordivmod(f, g)[1]
        f, g = g, r
    return f
...
def xorpowmod(a, e, f):
    ans, a = 1, xordivmod(a,f)[1]
    for k in bin(e)[2:]:
        ans = xordivmod(xormul(ans, ans), f)[1]
        if k == '1': ans = xordivmod(xormul(ans, a), f)[1]
    return ans

def xorirred(f):
    # Uses the Ben-Or algorithm to determine whether f is irreducible over F2.
    if f == 1: return False
    r = 2
    for d in range(1, (f.bit_length()-1)//2 + 1):
        r = xorpowmod(r, 2, f)
        s = r ^ 2
        g = xorgcd(f, s)
        if g != 1: return False
